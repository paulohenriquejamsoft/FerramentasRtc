...
namespace CategorizarProdutos.Repositorios
{
    public class NaturezaRepository
    {
        ...
        {
            var query = @"SELECT NATOPERACAO CODNATUREZA, DESCRNATUREZA,CODFISCAL CFOP, OP.ID AS OPID, OP.descricao_operacao OPDESCRICAO , SIT.CODIGO CSTIBSCBS, CLASS.CODIGO CCLASSTRIB
                        FROM NATOPER NAT
                        LEFT JOIN OPERACAOTRIBUTACAOFISCAL OTF (NOLOCK) ON OTF.IDOPERFISCAL=NAT.IdOperacaoFiscal AND OTF.TIPOREGRA=1
                        LEFT JOIN OPERACAO_FISCAL OP (NOLOCK) ON OP.ID=OTF.IDOPERFISCAL
